...
def action_get_repo(repo_path: str = "."):
    try:
...
def action_get_root_path(repo):
    return Path(repo.git.rev_parse("--show-toplevel"))

def action_load_pyproject_toml(root_path):
...
def action_get_branches(repo) -> dict:
    branches = {}
...
def action_discover_module(pyproject_toml, root_path) -> tuple[Path | None, Path | None]:
    """
    ...
    try:
        ...
        find_config = pyproject_toml.get("tool", {}).get("setuptools", {}).get("packages", {}).get("find", {})
    ...
    try:
        dynamic_config = pyproject_toml.get("tool", {}).get("setuptools", {}).get("dynamic", {})
    ...
    if not module_python_path:
        ...
        try:
            ...
            if project_name:
                ...
                temp_source_path = root_path / source_dir_name
    ...
    source_path = root_path / source_dir_name
...
def action_get_version(module: Path) -> str | None:
...
def action_set_version(module: Path, version: str) -> None:
    init_file = module / "__init__.py"
    lines = init_file.read_text().splitlines()
...
def action_update_changelog(root_path, version: str, description: str) -> None:
    changelog_file = root_path / "CHANGELOG.rst"
...
- actions"""
    changelog_file.write_text(changelog_file.read_text() + template)

def action_merge_to_main(repo, branch_name: str, commit_message: str) -> None:
    repo.git.checkout("main")
    repo.git.merge(branch_name, m=commit_message)
    return True, f"Merged {branch_name} to main"

def action_create_release_branch(repo, release_version: str, description: str) -> None:
    # This function will need to call other action_* functions
    root_path = action_get_root_path(repo)
    pyproject_toml = action_load_pyproject_toml(root_path)
    namespace, module = action_discover_module(pyproject_toml, root_path)

    if module is None:
        return False, "Module not found"

    repo.git.checkout("-b", release_version)
    action_set_version(module, release_version)
    action_update_changelog(root_path, release_version, description)

    repo.git.add(str(module / "__init__.py"), str(root_path / "CHANGELOG.rst"))
    repo.git.commit("-m", f"init release {release_version}\n{description}")

    try:
        repo.git.push("--set-upstream", "origin", release_version)
    except Exception as e:
        return False, f"Failed to push branch: {str(e)}"
    return True, f"\n{release_version} set and pushed to remote"

def action_create_tag(repo, tag_name: str, message: str) -> tuple[bool, str]:
    if repo.active_branch.name != 'main':
        return False, "Must be on 'main' branch to create a release tag."
    try:
        repo.create_tag(tag_name, message=message, force=False)
